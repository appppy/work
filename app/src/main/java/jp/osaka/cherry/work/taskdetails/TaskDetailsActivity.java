...
public class TaskDetailsActivity extends AppCompatActivity {
    ...
    public boolean onCreateOptionsMenu(final Menu menu) {
        switch (mDataSet.content) {
            case ARCHIVE: {
                getMenuInflater().inflate(R.menu.archive_details, menu);
                break;
            }
            case TRASH: {
                getMenuInflater().inflate(R.menu.trash_details, menu);
                break;
            }
            default: {
                getMenuInflater().inflate(R.menu.tasks_details, menu);
                break;
            }
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (mDataSet.content) {
            case ARCHIVE: {
                if (item.getItemId() == R.id.menu_unarchive) {// アーカイブ解除
                    mDataSet.content = CONTENT.INBOX;
                    setResult();
                    finish();
                    return true;
                }
            ...
            case TRASH: {
                if (item.getItemId() == R.id.menu_untrash) {// ゴミ箱から戻す
                    mDataSet.content = CONTENT.INBOX;
                    setResult();
                    finish();
                    return true;
            ...
            default: {
                switch (item.getItemId()) {
                    case R.id.menu_archive: {
                        // アーカイブ
                        mDataSet.content = CONTENT.ARCHIVE;
                        setResult();
                        finish();
                        return true;
                    }
                    case R.id.menu_trash: {
                        // ゴミ箱
                        mDataSet.content = CONTENT.TRASH;
                        setResult();
                        finish();
                        return true;
                    }
